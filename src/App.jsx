        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center relative z-10 pt-24 md:pt-32">
          <div className="lg:col-span-7 text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
                {/* Updated Button to navigate to doctors page */}
                <button 
                ...
                <button onClick={onNavigateHowitWorks} className="flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/30 text-white px-8 py-4 rounded-full font-bold hover:bg-white/20 transition-all duration-300 w-full sm:w-auto justify-center">
                How it works
                </button>
              </div>
          </div>
          ...
          <div className="lg:col-span-5 relative mt-8 lg:mt-0">
              <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-6 border border-white/50 shadow-2xl relative z-10 rotate-2 hover:rotate-0 transition-transform duration-500 cursor-default">
...
export default function App() {
  ...
  return (
    <main className="relative min-h-screen">
      {currentPage !== 'login' && currentPage !== 'doctors' && (
        <Navbar 
      ...
      {currentPage === 'home' && (
        <LandingPage 
          onNavigateHowitWorks={() => navigateToSection('how-it-works')}
          onFindSpecialist={() => navigateTo('doctors')} // - Trigger navigation to doctors
        />
      )}
      
      {currentPage === 'about' && <AboutPage onBack={() => navigateTo('home')} />}
      {currentPage === 'login' && <LoginPage onBack={() => navigateTo('home')} />}
      
      {/* ADDED THIS ROUTE 👇 */}
      {currentPage === 'doctors' && (
        <DoctorsPage onBack={() => navigateTo('home')} /> // - Render the frame
      )}
    </main>
  );
}