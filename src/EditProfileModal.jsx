...
// Added initialSection prop here
const EditProfileModal = ({ isOpen, onClose, user, onSave, initialSection = 'personal' }) => {
  ...
  // NEW: Update active section whenever the modal opens or initialSection changes
  useEffect(() => {
    if (isOpen) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onSave({ ...formData, age: age > 0 ? age : null });
      onClose();
    } catch (error) {
              ...
              {activeSection === 'medical' && (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="text" value={formData.allergies.join(', ')} onChange={(e) => setFormData(prev => ({ ...prev, allergies: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }))} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 outline-none transition-all" placeholder="e.g. Penicillin, Peanuts, Dust" />
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-bold text-slate-700">Chronic Conditions (comma separated)</label>
                    <input type="text" value={formData.chronicConditions.join(', ')} onChange={(e) => setFormData(prev => ({ ...prev, chronicConditions: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }))} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 outline-none transition-all" placeholder="e.g. Asthma, Hypertension, Diabetes" />
                  </div>
